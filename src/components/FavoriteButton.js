...
export default function FavoriteButton({ productId, style, onChanged, onRequireLogin }) {
  const { user } = useAuth();
  ...
  const handlePress = async () => {
    if (!user) {
      ...
      Alert.alert('Favoritos', 'Você precisa entrar na sua conta para usar essa função.');
      return;
    }
    try {
  ...
  return (
    <TouchableOpacity
      style={[styles.button, style, loading && styles.loading]}
      onPress={handlePress}
      disabled={loading}
...
const styles = StyleSheet.create({
  ...
  icon: { color: '#c0606a', fontSize: 25, lineHeight: 28 },
  active: { color: '#c0606a' },
  loading: { opacity: 0.5 },
});
