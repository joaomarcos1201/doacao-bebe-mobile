...
export default function MyOrdersScreen({ onBack }) {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      else if (apiErr?.status === 403) setError('Acesso negado. Você não tem permissão para ver seus pedidos.');
      ...
      else setError(apiErr?.message || 'Falha ao carregar pedidos.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <View style={s.container}>
      <View style={s.navbar}>
        ...
        <Text style={s.navTitle}>Meus Pedidos</Text>
      ...
      >
        {loading && !refreshing ? (
          <Text style={s.statusText}>Carregando pedidos...</Text>
        ) : error ? (
          ...
          <View style={s.statusBox}>
            <Text style={s.statusTitle}>Nenhum pedido encontrado</Text>
            <Text style={s.statusSubtitle}>Você ainda não fez pedidos no aplicativo.</Text>
          </View>
        ) : (
          orders.map((order) => (
            <TouchableOpacity key={order.id} style={s.card} activeOpacity={0.8} onPress={() => {}}>
              ...
              <View style={s.row}>
                ...
                <View style={s.metaBox}>
                  <Text style={s.metaLabel}>Frete</Text>
                  <Text style={s.metaValue}>R$ {order.valorFrete?.toFixed(2) ?? '0.00'}</Text>
                </View>
              </View>
              <View style={s.row}> 
                <View style={s.metaBox}>
                  <Text style={s.metaLabel}>Total</Text>
                  <Text style={s.metaValue}>R$ {order.valorTotal?.toFixed(2) ?? '0.00'}</Text>
                </View>
                <View style={s.metaBox}>
